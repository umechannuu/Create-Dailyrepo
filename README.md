...
2. 開始通知をSlackに送信（対象期間を表示）
3. 指定チャンネルから対象期間のメッセージを非同期で取得
4. Notionから直近の過去日報を取得し、文脈として活用
5. Gemini 2.5 Flash でチャンネルごとに並列要約・TODO提案を生成
6. Notionデータベースにチャンネルごとのページを並列作成
7. メッセージ内のURLを参考文献として自動添付
...
- 日報生成の開始・完了・エラー通知をSlackに送信
- 過去日報の自動参照（Notion APIから直近3日分を取得し、文脈としてGeminiに提供）
- AI要約による日報自動生成
- AI提案TODO（優先度・所要時間付き）
...
2. 処理開始時に「日報生成タスクを開始しました」通知をSlackに送信
3. バックグラウンドで並列処理実行（1-2分に短縮）
4. 完了後、Slackに通知

## 処理の流れ

1. スラッシュコマンド受信（FastAPI）、時間範囲をパース
2. BackgroundTasksでバックグラウンド処理開始、開始通知をSlackに送信
3. 指定期間のメッセージを全チャンネルから並列取得（httpx）
4. Notionから直近3日間の過去日報を並行取得
5. チャンネルごとにメッセージをグループ化
6. 各チャンネルを並列処理：
   - Gemini APIで要約・TODO提案生成（過去日報の文脈を含む）
   - URLを抽出して参考文献リストを作成
7. Notionにチャンネル別ページを並列作成（httpx）
8. 完了通知をSlackに送信
